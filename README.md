...
ВИДЕО на ЮТУБЕ https://www.youtube.com/watch?v=yySeKusFxw4
...
На чистом сервере `git` может быть не установлен. Выполните:
...
3. **Вариант конфига** — Full (все сервисы отдельно) или Small (объединённые группы)

Дальше всё автоматически. Установка занимает 2-5 минут.

## Шаг 5. Забрать конфиг клиента
...
При установке предлагается выбор:

| Вариант | Файл | Описание |
|---------|------|----------|
| **Full** | `full_config.yaml` | Каждый сервис — отдельная группа (Apple, Google, Google Play, Microsoft, Steam, Discord, Roblox, Twitch, Instagram, Facebook, Amazon, Tidal, Akamai, Fastly, intel...) |
| **Small** | `small_config.yaml` | Объединённые группы: Big Tech (Apple+Google+Microsoft+Amazon), Игры (Steam+Discord+Roblox+Twitch), Meta (Instagram+Facebook). Автовыбор лучшего прокси для Telegram и WhatsApp |

## Что делает install.sh

**Первый запуск (установка):**

1. Спрашивает IP сервера, URL подписки и вариант конфига
2. Обновляет систему и ставит зависимости
3. Скачивает и устанавливает mihomo (последняя версия с GitHub)
4. Копирует выбранный конфиг → `/etc/mihomo/config.yaml`
5. Подставляет сгенерированный секрет и URL подписки
6. Добавляет TUN-секцию для перехвата DNS
7. Создаёт systemd сервис для mihomo
8. Устанавливает AmneziaWG (PPA + DKMS модуль ядра)
9. Генерирует случайные параметры обфускации (уникальные для сервера)
10. Генерирует ключи сервера и первого клиента
11. Создаёт postup.sh/postdown.sh с правилами iptables
12. Включает ip_forward и route_localnet (переживает ребут)
13. Запускает mihomo и awg0, включает автозапуск

**Повторный запуск (меню управления):**

```
1) Диагностика              — запуск diagnose.sh
2) Информация о Mihomo UI   — URL и секрет для входа
3) Сменить секрет Mihomo     — генерация нового секрета + перезапуск
4) Редактировать конфиг      — открывает /etc/mihomo/config.yaml в nano
5) Восстановить конфиг       — выбор Full/Small из репозитория, с возможностью
                                сохранить текущий секрет и URL подписки
6) Полная переустановка      — бэкап + установка с нуля
```
...
Запустите `install.sh` → выберите пункт **6) Полная переустановка**.
Скрипт сделает бэкап в `/root/backup-vpn-ДАТА/` и поставит всё заново
с новыми ключами и параметрами обфускации.
...
```
full_config.yaml     — конфиг mihomo Full (все сервисы отдельно)
small_config.yaml    — конфиг mihomo Small (объединённые группы)
install.sh           — установка + меню управления
